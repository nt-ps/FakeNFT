...
final class MyNFTPresenter: MyNFTPresenterProtocol {
    ...
    // MARK: Initializers
    init(view: MyNFTViewProtocol, profileService: ProfileServiceProtocol? = nil) {
        self.view = view
        if let profileService = profileService {
            self.profileService = profileService
        } else {
            self.profileService = USE_EMPTY_MOCK_DATA ? EmptyMockProfileService.shared : MockProfileService.shared
        }
    }
