...
//
//       - Добавил completion в параметры метода. На put-запросы
//         тоже нужно получать ответ, успешно ли все прошло или нет,
//         чтобы протягивать ошибку на экран.
//       - Заменил входной параметр [NFT] на массив [String]. В запросе
//         мы отправляем массив ID, однако на каждом экране модель их хранения
//         может отличаться, и ответственность за получение массива ID пусть
//         лежит на классе, который хочет отправить запрос.
//
